...
def get_pages(html):
    ...
    pages_name = soup.find_all('a', class_="icMQ_ f-test-link- _3dPok")
    obraovans = []
    experienses = []
    ages = []
    compensations = []
    ...
    try:
        for i in range(len(pages_name)):
            ########зарплата##########
            compensations.append("".join(pages_name[i].find('span', class_="_1h3Zg _3Q_Pz ADNB4 _1BoTZ _3ZLBy")
                                     .find("span").find_all("span")[0].text.split()))
            ##########################
            ########возраст###########
            if len(pages_name[i].find('span', class_="_1h3Zg z15vz _1xK5K _3ZLBy").find("span").find_all("span")) > 0:
                ages.append(pages_name[i].find('span', class_="_1h3Zg z15vz _1xK5K _3ZLBy").find("span")
                        .find_all("span")[0].text.split(" ")[0])
            else:
                ages.append("30")
            ##########################
            #########опыт работы#######
            opt_buf = pages_name[i].find('span', class_="_1h3Zg z15vz _1xK5K _3ZLBy").find("span")\
                .find('span', class_= "_3AnJT")
            int_buf = 0
            if opt_buf:
                if len(opt_buf) > 0:
            ...
            obrs = soup1.find_all('div', class_="_1h3Zg z15vz _1xK5K _21a7u _2SvHc")
